...
{
    tcp_transport( int fd, transport_callback* cb )
    : _fd( fd ),
      _callback( cb )
    {
    }
    ...
    int _fd;
    decoder _decoder;
    transport_callback* _callback;
};

// -----------------------------------------------------------------------------
struct tcp_transport_callback : public ::hulk::tcp_callback
{
    tcp_transport_callback( transport_callback& cb )
    : _callback( cb )
    {
    }
...
{
    session_callback( shared_ptr< session > s )
    : _session( s ),
      _transport( 0 )
    {
    }
...
// -----------------------------------------------------------------------------
void ::hulk::fix::tcp_event_loop::new_initiator( const char* host, int port, shared_ptr< session >& s )
{
...
void ::hulk::fix::tcp_event_loop::new_initiator( const char* host, int port, transport_callback& cb )
{
    int fd = ::hulk::tcp_non_blocking( ::hulk::tcp_connect( host, port ) );
...
void ::hulk::fix::tcp_event_loop::new_acceptor( int port, transport_callback& cb )
{
